...
public class TikTokCommand implements CommandExecutor {
    Main main;
    public TikTokCommand(Main main) {
        this.main = main;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!main.config.getBoolean("tiktok.enable")) {
        ...
        if (args.length == 0) {
            if (player.hasPermission("mycontact.tiktok")) {
                TextComponent msg = new TextComponent(main.getString("tiktok.message")
                        .replace("&", "§"));
                msg.setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, main.getString("tiktok.link")));
        ...
        if (args.length >= 1) {
            if (player.hasPermission("mycontact.tiktok.config")) {
                if (args.length >= 3) {
                    if (args[0].equalsIgnoreCase("config")) {
                        if (args[1].equalsIgnoreCase("link")) {
                            main.config.set("tiktok.link", args[2]);
                            ...
                            main.config.set("tiktok.message", message);
                        }
                    ...
                    if (args[0].equalsIgnoreCase("disable")) {
                        main.config.set("tiktok.enable", false);
                        try {
                            main.config.save(file);
                            main.config.load(file);
                        ...
                        player.sendMessage("§c/tiktok config link/message <link>");
                    }
