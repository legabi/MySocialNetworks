...
public class TwitterCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!main.config.getBoolean("twitter.enable")) {
        ...
        if (args.length == 0) {
            if (player.hasPermission("mycontact.twitter")) {
                TextComponent msg = new TextComponent(main.getString("twitter.message")
                        .replace("&", "§"));
                msg.setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, main.getString("twitter.link")));
        ...
        if (args.length >= 1) {
            if (player.hasPermission("mycontact.twitter.config")) {
                if (args.length >= 3) {
                    if (args[0].equalsIgnoreCase("config")) {
                        if (args[1].equalsIgnoreCase("link")) {
                            main.config.set("twitter.link", args[2]);
                            ...
                            main.config.set("twitter.message", message);
                        }
                    ...
                    if (args[0].equalsIgnoreCase("disable")) {
                        main.config.set("twitter.enable", false);
                        try {
                            main.config.save(file);
                            main.config.load(file);
                        ...
                        player.sendMessage("§c/twitter config link/message <link>");
                    }
